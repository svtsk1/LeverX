class Version:
    """Переопределены три метода сравнения версий(lt, gt, neq).
    ...
    def __gt__(self, other):
        """Сравнивает попарно версии в цикле.
        Если в паре первое число больше второго, то и весь номер версии больше, соответственно возвращает True.
        Если в паре первое число меньше второго, то и весь номер версии меньше, соответственно возвращает False.
        Если в паре числа равны друг-другу, то переходит к следующей паре.
        Если цикл прошёлся по всем получившимся парам, то сравниваются количество элементов в номере версий:
            Если в первой версии количество элементов больше, то она больше второй версии."""
        couples = list(zip(self.version, other.version))
        for couple in couples:
            if couple[0] > couple[1]:
                return True
    ...
    def __ne__(self, other):
        """Сравнивает попарно версии в цикле.
        Если в паре первое число меньше или больше второго, то номера версий не равны, соответственно возвращает True.
        Если в паре числа равны друг-другу, то переходит к следующей паре.
        Если цикл прошёлся по всем получившимся парам, то сравниваются количество элементов в номере версий:
            Если количество элементов в парах не одинаково, то онаи не равны друг-другу."""
        couples = list(zip(self.version, other.version))
        for couple in couples:
            if couple[0] < couple[1]:
                return True
